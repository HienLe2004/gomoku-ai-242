...
class Cell:
    ...
    def draw(self):
        ...
        if self.type == Cell_Type.BLACK:
            ...
            #     text_color = Cell.white_color
            if self.grid.last_move['position'] == self.grid_position:
                pygame.draw.rect(surface=self.grid.grid_surf, color=self.grid_color, rect=self.cell_rect.scale_by(0.05, 0.2).move(self.size[0]/2,self.size[0] * 0.4))
                pygame.draw.rect(surface=self.grid.grid_surf, color=self.grid_color, rect=self.cell_rect.scale_by(0.2, 0.05).move(self.size[0] * 0.4,self.size[0]/2))
                pygame.draw.rect(surface=self.grid.grid_surf, color=self.grid_color, rect=self.cell_rect.scale_by(0.05, 0.2).move(-self.size[0]/2,self.size[0] * 0.4))
                pygame.draw.rect(surface=self.grid.grid_surf, color=self.grid_color, rect=self.cell_rect.scale_by(0.2, 0.05).move(-self.size[0] * 0.4,self.size[0]/2))
                pygame.draw.rect(surface=self.grid.grid_surf, color=self.grid_color, rect=self.cell_rect.scale_by(0.05, 0.2).move(self.size[0]/2,-self.size[0] * 0.4))
                pygame.draw.rect(surface=self.grid.grid_surf, color=self.grid_color, rect=self.cell_rect.scale_by(0.2, 0.05).move(self.size[0] * 0.4,-self.size[0]/2))
        ...
        if self.is_potential:
            pygame.draw.circle(surface=self.grid.grid_surf, color=(255,200,150), center=self.position, radius=self.size[0] * 0.3)
